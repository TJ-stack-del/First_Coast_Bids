...
- **Where it lives:** `.theme-press` in `app/globals.css` remaps the same color tokens inside `app/(marketing)/layout.tsx`'s wrapper, and that layout loads Newsreader and Archivo through `next/font`. Nothing outside the wrapper is affected. The landing page's own layout is `components/marketing/press.module.css`.
