...
## Marketing theme: "Braun + Press" (public site and client area)

Since 2026-09-22 the public marketing pages (everything under `app/(marketing)/`: home, pricing, FAQ, gallery, blog, quiz, contact, privacy, terms) use a second, scoped theme. Since 2026-09-23 the sign-up path a visitor reaches from those pages uses it too: intake (`/intake`), log in (`/login`) and password reset (`/reset-password`), each through its own `layout.tsx`. Since 2026-09-26 the client area uses it too: your bids (`/dashboard`), company profile (`/dashboard/profile`) and compliance vault (`/dashboard/compliance`), through `app/dashboard/layout.tsx`. The admin still uses the Compliance Ledger system described in the rest of this file.

- **Where it lives:** `.theme-press` in `app/globals.css` remaps the same color tokens, and `app/press-theme.ts` exports the wrapper class with Newsreader and Archivo loaded through `next/font`. Inside the theme, buttons that borrow the heading font read in Archivo, text fields get a visible border, and large rounded corners are squared off to 4px. Nothing outside the wrapper is affected. The landing page's own layout is `components/marketing/press.module.css`.
...
- **Inner pages** (since 2026-09-23): a left-aligned serif page head (only the landing hero is centred), then content on hairline ledgers under a heavy 2px top rule. Pricing reuses the landing page's spec table (`components/marketing/PricingSpec.tsx`). FAQ shows every answer, with no accordion. Gallery and Blog are ledgers of rows, not cards. Forms and the quiz sit on a single raised sheet. Privacy and Terms use a reading column. Shared styles are in `components/marketing/press.module.css`.
- **Client area** (since 2026-09-26, spec `docs/superpowers/specs/2026-09-26-client-area-redesign-design.md`): a slim top bar (Your bids, Company profile, Compliance vault; a bottom tab bar on phones) instead of the admin's sidebar. The dashboard opens with a "Your bids" page head, then **Needs you** (one row per waiting bid with its one action, from `lib/dashboard/client-tasks.ts`), then **All your bids** as one-line rows that open one at a time to the bid's details (`app/dashboard/BidLedger.tsx`), a one-line **Your file** summary, and the bid process reminders closed by default. Warnings that must never be hidden (below the wage-law floor, a mandatory site visit) show in red on the closed row. Profile and vault sections sit under a heavy 2px rule with a serif h2.
- **One main action:** "Start a pilot bid" (`/intake?package=pilot`) in the header, the hero, the final call to action and the quiz result, while the free Pilot cohort is open.
